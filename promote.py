...
def get_rougeL(run_id: str) -> float:
    try:
        run = client.get_run(run_id)
        return float(run.data.metrics.get("rougeL", 0.0))
    except Exception:
        return 0.0
...
def main():
    ...
    if args.version is not None:
        ...
        try:
            ...
            print(f"Current staging: v{target_v} (rougeL: {get_rougeL(mv.run_id):.4f})")
    ...
    try:
        ...
        print(
            f"Current production: v{current_prod.version} "
            f"(rougeL: {get_rougeL(current_prod.run_id):.4f})"
        )
    except Exception:
    ...
    if deploy_url:
        try:
            ...
            print(f"Warning: promotion succeeded, but reload failed: {e}")
