...
MLFLOW_TRACKING_URI = os.environ.get("MLFLOW_TRACKING_URI", "http://129.114.27.29:8002")

def get_latest_run(experiment_name):
    """Get latest run metrics from MLflow server."""
    try:
        import mlflow
        ...
        print(f"MLflow error for {experiment_name}: {e}")
        return {}, {}

def get_data():
    ing_m, ing_t   = get_latest_run("data_quality_ingestion")
    trn_m, trn_t   = get_latest_run("data_quality_training_set")
    drft_m, drft_t = get_latest_run("data_quality_drift_monitor")
    fb_m, fb_t     = get_latest_run("data_quality_feedback")

    ing_pass = int(ing_m.get("pass_count", 0))
    ing_warn = int(ing_m.get("warning_count", 0))
    ing_quar = int(ing_m.get("quarantine_count", 0))
    ing_gate = ing_t.get("gate_result", "UNKNOWN")

    train_count = int(trn_m.get("train_count", 140))
    eval_count  = int(trn_m.get("eval_count", 7))
    synth_ratio = trn_m.get("synthetic_ratio", 0.75)
    train_gate  = trn_t.get("gate_result", "PASSED")

    drift_score  = round(drft_m.get("max_drift_score", 0.0), 4)
    drift_status = drft_t.get("drift_status", "OK")
    n_alerts     = int(drft_m.get("n_alerts", 0))

    collected = int(fb_m.get("collected", 0))
    rejected  = int(fb_m.get("rejected", 0))

    total     = ing_pass + ing_warn + ing_quar
    pass_rate = round(ing_pass / max(total, 1) * 100, 1)

    return {
        "ing_pass": ing_pass, "ing_warn": ing_warn,
        "ing_quar": ing_quar, "ing_gate": ing_gate,
...
def dashboard():
    d = get_data()
    drift_label = "CRITICAL" if d["drift_score"] > 0.60 else ("WARNING" if d["drift_score"] > 0.30 else "No drift detected")
    html = f"""<!DOCTYPE html>
...
.badge.warn{{color:#BA7517;background:#FEF3CD}}
.badge.critical{{color:#A32D2D;background:#FFE6E6}}
...
.msub.warn{{color:#BA7517}}
.msub.bad{{color:#A32D2D}}
...
.mlflow-link{{font-size:12px;color:#185FA5;text-decoration:none}}
.ref{{font-size:12px;color:#888;text-align:right;margin-top:16px}}
</style></head><body>
<div class="header">
  <div>
    <h1>Data quality &amp; drift monitor</h1>
    <p class="sub">Project 30 — MeetingBank | {d["timestamp"]} | 
      <a class="mlflow-link" href="{d["mlflow_uri"]}" target="_blank">MLflow ↗</a>
    </p>
  </div>
  <div class="badge {'warn' if d['drift_score'] > 0.30 else ''}">
    <span class="dot"></span>Drift: {d["drift_status"]}
  </div>
</div>
<div class="grid4">
  <div class="mcard">
    <p class="mlabel">Total meetings</p>
    <p class="mval">{d["total"]}</p>
    <p class="msub">Ingested from MeetingBank</p>
  </div>
  <div class="mcard">
    <p class="mlabel">Pass rate</p>
    <p class="mval">{d["pass_rate"]}%</p>
    <p class="msub {'bad' if d['ing_quar'] > 0 else ''}">{d["ing_quar"]} quarantined — gate: {d["ing_gate"]}</p>
  </div>
  <div class="mcard">
    <p class="mlabel">Max drift score</p>
    <p class="mval">{d["drift_score"]}</p>
    <p class="msub {'warn' if d['drift_score'] > 0.30 else ''}">{drift_label}</p>
  </div>
  <div class="mcard">
    <p class="mlabel">Training samples</p>
    <p class="mval">{d["train_count"]}</p>
    <p class="msub">{d["real_count"]} real + {d["synth_count"]} synthetic</p>
  </div>
</div>
<div class="grid2">
  <div class="card">
    ...
    <div style="position:relative;height:200px">
      <canvas id="c1" role="img" aria-label="Ingestion results"></canvas>
    </div>
  </div>
  <div class="card">
    ...
    <div style="position:relative;height:200px">
      <canvas id="c2" role="img" aria-label="Training composition"></canvas>
    </div>
  </div>
</div>
<div class="grid2">
  <div class="card full">
    ...
    <p class="csub">WARNING at 0.30 — CRITICAL at 0.60 — checks every 5 min</p>
    <div class="legend">
    ...
    <div style="position:relative;height:200px">
      <canvas id="c3" role="img" aria-label="Drift over time"></canvas>
    </div>
  </div>
</div>
<div class="grid2">
  ...
  <div class="card">
    ...
    <div style="position:relative;height:180px">
      <canvas id="c4" role="img" aria-label="Feedback"></canvas>
    </div>
  </div>
</div>
<div class="card" style="margin-top:16px">
  <p class="ctitle">Pipeline activity</p>
  <div id="log"></div>
</div>
<p class="ref">
  Auto-refreshes every 60s · <a href="javascript:location.reload()">Refresh now</a> · 
  <a href="{d["mlflow_uri"]}" target="_blank">Open MLflow ↗</a>
</p>
<script>
const checks=[
...
checks.forEach(c=>{{
  ...
  d.className='crow';
  d.innerHTML=`<div class="cleft"><span class="cicon">✓</span><span>${{c.name}}</span></div><span style="font-size:12px;color:#888">${{c.detail}}</span>`;
  cl.appendChild(d);
}});
const acts=[
  ...
  {{t:'Recent',e:'Drift check',d:'Status: {d["drift_status"]} — score {d["drift_score"]}',c:'#185FA5'}},
  {{t:'Recent',e:'Feedback collection',d:'{d["collected"]} transcripts collected',c:'#BA7517'}},
  {{t:'Running',e:'Drift monitor active',d:'Continuous — every 5 min → MLflow {d["mlflow_uri"]}',c:'#185FA5'}},
];
const lg=document.getElementById('log');
acts.forEach(a=>{{
  const d=document.createElement('div');
  d.className='lrow';
  d.innerHTML=`<span class="ltime">${{a.t}}</span><span class="ldot" style="background:${{a.c}}"></span><div><p class="levent">${{a.e}}</p><p class="ldetail">${{a.d}}</p></div>`;
  lg.appendChild(d);
}});
